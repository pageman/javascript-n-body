		...
		for (var i=0; i<system.length; i++)
			{
				...
				//console.log("call for 'gravi'");
				var qdist = (body.x - system[i].x)*(body.x - system[i].x) + (body.y - system[i].y)*(body.y - system[i].y);
				if (qdist > 0)
					{
...
function collisions(system_object)
{
	...
	function isThereHit(body1, body2)
		{
			if (Math.pow((body1.x - body2.x),2) + Math.pow((body1.y - body2.y),2) < Math.pow( body1.radii + body2.radii, 2 )) 
			{ return true } 
			else 
			{ return false; }
		}
					...
					if (isThereHit(b1,b2)) 
						{
							...
							var c = 1 / Math.sqrt( (b1.x-b2.x)*(b1.x-b2.x) + (b1.y-b2.y)*(b1.y-b2.y) ); //normalize vectors
							var r = [ (b2.x-b1.x)*c, (b2.y-b1.y)*c ]; //normalized radius - vector
							var shiftx = b1.vx;
							var shifty = b1.vy;
							b1.vx=0;
							b1.vy=0;
							...
							//till there works fine;
							
							var delta = b1.radii + b2.radii - Math.sqrt( (b1.x-b2.x)*(b1.x-b2.x) + (b1.y-b2.y)*(b1.y-b2.y) );
							b2.x += delta * r[0]/2;
							b2.y += delta * r[1]/2;
...
//this class performs all physix calculation;
				...
				for (var i=0; i<hres; i++)
					{
						for (var j=0; j<vres; j++)
							{
								this.addCell( new Cell( i*this.hres, (i+1)*this.hres, j*this.vres, (j+1)*this.vres ) ) 
							}
					}
				//console.log("cell count is "+this.cells.length)
			}; //call on start
		this.initCells();
		this.sort = function(body) // takes body obj and inserts it in right cell
			{
				...
				//this.cells[j*hres + i].set.push(body);
		...
		this.moveConstrainsBody = function(body)
			{
				var bounce = 0.7;
				var frict = 0.95;
				if (body.x < body.radii) {body.x = body.radii; body.vx = -bounce*body.vx; body.vy*=frict; body.moved = true;};
				if (body.y < body.radii) {body.y = body.radii; body.vy = -bounce*body.vy; body.vx*=frict;body.moved = true;};
				if (body.x > (system_object.box.w-body.radii)) 
				{ body.x = system_object.box.w-body.radii; body.vx = -bounce*body.vx; body.vy*=frict; body.moved = true;};
				if (body.y > (system_object.box.h-body.radii)) 
				{ body.y = system_object.box.h-body.radii; body.vy = -bounce*body.vy; body.vx*=frict; body.moved = true;};
			}
...
window.onload = function()
	{
		system = new System();
		for (var i = 0; i<200; i++) { system.push( new Body(Math.random()*700, Math.random()*400, 5) ); } //Filling system
